...
import {checkHasPermission, fetchInvestmentHubApprovedReviewsListings, isSmallScreen} from "@/services/Helpers";
...
const isLoading = computed({
    get: ()=> store.state.investmentHub.isApprovedReviewsListingsLoading,
    set: (value)=> store.commit("investmentHub/SET_IS_APPROVED_REVIEWS_LISTINGS_LOADING", value)
});
const listings = computed(()=> store.state.investmentHub.approvedReviewsListings);
    ...
    <div class="table-responsive m-t-10" v-loading="isLoading">
        <table class="table">
            ...
            <tr v-if="listings.length" v-for="(listing, index) in listings"
                :key="'investment_hub-public-reviewed-listings-'+index"
