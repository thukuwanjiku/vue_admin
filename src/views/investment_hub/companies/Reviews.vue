    ...
    let newLinks = JSON.parse(JSON.stringify(paginationLinks.value))
            ...
            .map(entry => {
                if(entry.label.toLowerCase().includes('previous')){
                    entry.url = pageNo == 1 ? null : `${apiRoutes.INVESTMENT_HUB_COMPANY_REVIEWS}?page=${currentPage - 1}`
                }
                if(entry.label.toLowerCase().includes('next')){
                    entry.url = pageNo == totalPages.value ? null : `${apiRoutes.INVESTMENT_HUB_COMPANY_REVIEWS}?page=${currentPage + 1}`
                }
                return entry;
            });
