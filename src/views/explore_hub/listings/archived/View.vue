...
* */
onMounted(()=>{
    if(store.state.exploreHub.viewedArchivedListing == null){
        return router.back();
    }

    //copy listing to local variable
    listing.value = JSON.parse(JSON.stringify(store.state.exploreHub.viewedArchivedListing));
    ...
    <div class="row" v-if="Object.keys(listing).length" v-loading="isLoading">
        <div class="col-sm-12 mb-3 d-inline-flex align-items-center"
            ...
            <div class="p-1 m-r-10 d-flex align-items-center flex-wrap"
                 v-if="hasPermissionsWhichContain(['explore_hub.archived_listings.restore', 'explore_hub.archived_listings.delete'])">
                <template v-if="!isSmallScreen">
                    <el-button v-if="checkHasPermission('explore_hub.archived_listings.restore')" @click="confirmRestore" type="primary" :icon="RefreshLeft" text bg>Restore Listing</el-button>
                    <el-button v-if="checkHasPermission('explore_hub.archived_listings.delete')" @click="confirmDelete" type="danger" :icon="Delete" text bg>Delete Listing</el-button>
                </template>
                ...
                <el-dropdown trigger="click" @command="handleActionsClick" v-if="isSmallScreen">
                    ...
                    <template #dropdown>
                        <el-dropdown-menu>
                            <el-dropdown-item v-if="checkHasPermission('explore_hub.archived_listings.restore')" command="restore" :icon="RefreshLeft">Restore Listing</el-dropdown-item>
                            <el-dropdown-item v-if="checkHasPermission('explore_hub.archived_listings.delete')" command="delete" :icon="Delete">Delete Listing</el-dropdown-item>
                        </el-dropdown-menu>
                    </template>
                </el-dropdown>
            </div>
