...
function fetchListings(){
    ...
    //make api call
    api.post(apiRoutes.EXPLORE_HUB_REPORTED_LISTINGS)
            .then(response => {
...
function fetchReports(url=null){
    //set api endpoint
    if(!url) url = apiRoutes.EXPLORE_HUB_LISTING_REPORTS;
    ...
    let newLinks = JSON.parse(JSON.stringify(paginationLinks.value))
            ...
            .map(entry => {
                if(entry.label.toLowerCase().includes('previous')){
                    entry.url = pageNo == 1 ? null : `${apiRoutes.EXPLORE_HUB_LISTING_REPORTS}?page=${currentPage.value - 1}`
                }
                if(entry.label.toLowerCase().includes('next')){
                    entry.url = pageNo == totalPages.value ? null : `${apiRoutes.EXPLORE_HUB_LISTING_REPORTS}?page=${currentPage.value + 1}`
                }
                return entry;
            });
        ...
        <div class="table-responsive m-t-10">
            <table class="table">
                ...
                <tr v-if="listings.length" v-for="(listing, index) in listings"
                    :key="'explore_hub-reported-listings-'+index"
