...
import {
    checkHasPermission,
    fetchExploreHubActiveListings, fetchExploreHubApprovedReviewsListings,
    isSmallScreen
...
function fetchReviews(){
    ...
    //make api call
    api.get(apiRoutes.EXPLORE_HUB_PENDING_REVIEWS)
            .then(response => {
                //store reviews
...
function approveReview(payload){
    ...
    //make api call
    api.post(apiRoutes.EXPLORE_HUB_APPROVE_PENDING_REVIEW, payload)
            .then(response => {
...
function rejectReview(payload){
    ...
    //make api call
    api.post(apiRoutes.EXPLORE_HUB_REJECT_PENDING_REVIEW, payload)
            .then(response => {
                //refresh pending reviews list
                fetchReviews();
...
<template>
        ...
        <div class="table-responsive m-t-10">
            <table class="table">
                ...
                <tr v-if="reviews.length" v-for="(review, index) in reviews"
                    :key="'explore_hub-pending-reviews-'+index"
                        ...
                        <el-dropdown trigger="click" @command="handleEntryAction">
                            ...
                            <template #dropdown>
                                <el-dropdown-menu>
                                    ...
                                    <el-dropdown-item v-if="checkHasPermission('explore_hub.reviewed_listings.approve')" :command="{action:'approve',review}">Approve</el-dropdown-item>
                                    <el-dropdown-item v-if="checkHasPermission('explore_hub.reviewed_listings.approve')" :command="{action:'reject',review}">Reject</el-dropdown-item>
                                </el-dropdown-menu>
                            </template>
                        </el-dropdown>
                    </td>
    ...
    <el-dialog
        ...
        <div class="row p-2" v-loading="isLoading">
            ...
            <template v-if="checkHasPermission('explore_hub.reviewed_listings.approve')">
